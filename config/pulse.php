...
return [
    ...
    |
    | Never leave this null or blank in this app. routes/tenant.php is registered
    | by TenancyServiceProvider::mapRoutes() with no domain constraint, so a route
    | without one of its own resolves on *every* operator subdomain — an empty
    | value here would publish the dashboard at operatorname.yourdomain.com/pulse.
    | Pinning it to the admin host keeps it on the central domain, where the
    | viewPulse gate (AppServiceProvider) is the only way in.
    |
    | The fallbacks mirror tenancy.admin_domain so the safe value applies even when
    | nobody sets PULSE_DOMAIN. They repeat that literal rather than reading it with
    | config(), because config files load alphabetically — pulse.php is evaluated
    | before tenancy.php, so config('tenancy.admin_domain') would be null here.
    | Keep the two in step if the admin host default ever changes.
    |
    | First *non-blank* value wins, rather than env()'s own default argument: a
    | declared-but-empty `PULSE_DOMAIN=` returns '' rather than null, so the default
    | would never fire — and Laravel treats an empty domain as *no* domain
    | constraint, silently producing the exact unpinned route this whole note exists
    | to prevent. Testing for blankness covers both the unset and the blank case.
    |
    */

    'domain' => Arr::first(
        [env('PULSE_DOMAIN'), env('ADMIN_PANEL_DOMAIN'), 'admin.localhost'],
        fn (mixed $domain): bool => filled($domain),
    ),
    ...
    */
    ...
    'recorders' => [
        ...
        // The only recorder not fed by application traffic: CPU, memory and disk are
        // sampled by the long-running `php artisan pulse:check` daemon, which needs a
        // supervisor entry of its own (the scheduler cannot host it — it never exits).
        // Without that process the Servers card renders, stays permanently empty, and
        // gives no hint why. Run it in production or drop this recorder; an empty card
        // is worse than an absent one, because it reads as "no load".
        Servers::class => [
