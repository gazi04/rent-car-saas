 ...
 *
 * PII stance for this app: bookings, the customer directory and the storefront
 * concierge all handle real personal data (names, phone numbers, email addresses,
 * free-text visitor questions). Two settings keep that out of a third-party SaaS:
 *
 *  - `send_default_pii` stays false, which is what stops the SDK attaching request
 *    bodies, cookies and client IPs. Livewire update payloads carry full component
 *    state — the booking form and the concierge transcript — so this is the setting
 *    that matters most. Do not enable it.
 *  - `sql_bindings` is hard-coded false in both the breadcrumb and tracing sections
 *    below, deliberately not env-driven: the bindings on a bookings INSERT are the
 *    customer's name, phone and email verbatim.
 *
 * Leaving the DSN unset makes the whole SDK a no-op, which is the local/test default.
 *
...
return [
    ...
    'breadcrumbs' => [
        ...
        // Not env-driven on purpose — see the PII note at the top of this file.
        'sql_bindings' => false,
