...
@if ($this->isEnabled)
    <div
        ...
        class="fixed bottom-4 end-4 z-50 pb-[env(safe-area-inset-bottom)] print:hidden"
    ...
    >
        ...
        <button
            ...
            class="group relative flex size-14 items-center justify-center rounded-full bg-primary text-on-primary shadow-lg transition hover:scale-105 hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
            aria-label="{{ __('booking.concierge_launcher') }}"
        >
            <span class="absolute -top-0.5 -end-0.5 h-3.5 w-3.5 rounded-full border-2 border-surface-raised bg-positive"></span>
            ...
            <span class="pointer-events-none absolute end-full me-3 whitespace-nowrap rounded-control bg-ink px-2.5 py-1.5 text-xs font-medium text-ink-inverse opacity-0 shadow-lg transition group-hover:opacity-100">
        ...
        <div
            ...
            class="fixed inset-x-0 bottom-0 top-16 flex flex-col overflow-hidden rounded-t-panel border border-line bg-surface-raised shadow-2xl sm:static sm:h-[32rem] sm:max-h-[calc(100dvh-6rem)] sm:w-[min(24rem,calc(100vw-2rem))] sm:rounded-panel"
        >
            <div class="flex shrink-0 items-center gap-3 bg-primary px-4 py-3.5 text-on-primary">
                <span class="flex size-9 shrink-0 items-center justify-center rounded-full bg-on-primary/15">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" class="h-5 w-5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M8.625 12a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H8.25m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H12m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.76 9.76 0 0 1-2.555-.337A5.972 5.972 0 0 1 5.41 20.97a5.969 5.969 0 0 1-.474-.065 4.48 4.48 0 0 0 .978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25Z" />
                    </svg>
                </span>
                <div class="min-w-0 flex-1">
                    <p id="concierge-title" class="truncate text-sm font-semibold">{{ __('booking.concierge_title') }}</p>
                    <p class="truncate text-xs text-on-primary/75">{{ __('booking.concierge_subtitle') }}</p>
                </div>
                <button type="button" x-on:click="open = false" class="flex size-11 shrink-0 items-center justify-center rounded-full text-on-primary/80 transition hover:bg-on-primary/10 hover:text-on-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-on-primary/50" aria-label="{{ __('booking.concierge_close') }}">
            ...
            <div x-ref="scrollArea" class="flex flex-1 flex-col gap-3 overflow-y-auto bg-surface px-4 py-4 text-sm">
                <p class="max-w-[85%] rounded-panel rounded-bl-md bg-surface-raised px-3.5 py-2.5 text-ink-muted shadow-sm">
                    {{ __('booking.concierge_greeting') }}
                </p>

                @foreach ($messages as $message)
                    @if ($message['role'] === 'user')
                        <p class="ms-auto max-w-[85%] rounded-panel rounded-br-md bg-primary px-3.5 py-2.5 text-on-primary shadow-sm">
                            {{ $message['content'] }}
                        </p>
                    @else
                        <p class="me-auto max-w-[85%] rounded-panel rounded-bl-md bg-surface-raised px-3.5 py-2.5 text-ink shadow-sm">
                            {{ $message['content'] }}
                        </p>
                    @endif
                @endforeach

                <p x-show="pending" x-cloak x-text="pending" class="ms-auto max-w-[85%] rounded-panel rounded-br-md bg-primary px-3.5 py-2.5 text-on-primary shadow-sm"></p>

                <div wire:loading wire:target="ask" class="me-auto flex items-center gap-1 rounded-panel rounded-bl-md bg-surface-raised px-4 py-3 shadow-sm">
                    <span class="h-1.5 w-1.5 animate-bounce rounded-full bg-ink-faint [animation-delay:-0.3s]"></span>
                    <span class="h-1.5 w-1.5 animate-bounce rounded-full bg-ink-faint [animation-delay:-0.15s]"></span>
                    <span class="h-1.5 w-1.5 animate-bounce rounded-full bg-ink-faint"></span>
                </div>
            </div>
            ...
            <form
                ...
                class="shrink-0 border-t border-line bg-surface-raised p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]"
            >
                <div class="flex items-end gap-2">
                    <textarea
                        ...
                        class="min-h-11 flex-1 resize-none rounded-panel border border-line bg-surface px-3.5 py-2.5 text-sm text-ink placeholder:text-ink-faint focus:border-primary focus:bg-surface-raised focus:outline-none focus:ring-2 focus:ring-primary/30"
                    ></textarea>
                    <button
                        type="submit"
                        wire:loading.attr="disabled"
                        wire:target="ask"
                        class="flex size-11 shrink-0 items-center justify-center rounded-full bg-primary text-on-primary shadow-sm transition hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 disabled:opacity-50"
                ...
                @error('question')
                    <p class="mt-1.5 text-xs text-critical">{{ $message }}</p>
                @enderror
            </form>
        </div>
    </div>
